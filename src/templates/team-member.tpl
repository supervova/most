...
function getposts() {
	...
	$.ajax({
		...
		type: 'GET',
		async: true,
		data: {community: '{$community}', publishtime: publishtime, attached: attached, blogcatid: '{$blog_cat_id}', blogtag: '{$blog_tag}', blogdate: '{$blog_date}', blogsearch: '{$blog_search}', byuid: {$byuid}, posttype: '0,1', authorid: {$authorid} },
...
<h2 class="h2-author cc-title-section cc-push-text">Author</h2>
<div class="author-page">
<img alt="" src="{$authorsavatar}" class="author"><div class="div-block-5"><h1 class="h2-copy cc-title-section">{$authorname}</h1><div class="team-member-role-page">{$authorbio}</div></div></div></div>

{$blogsearchhtml}
				<span class="endposts"><button class="btn btn-default" onclick="getposts()">Show more...</button></span>
				<div id="postsblock">
				</div>
				<div id="loadingposts" style="display: none;text-align: center;"><br /><br /><span>Loading...</span><br /><img src="/images/loading.gif" alt="Loading..." width="30"><br /><br /></div>
